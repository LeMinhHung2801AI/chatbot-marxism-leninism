...
import marxBg from '@/assets/backgrounds/freedom-alive.png';
...
const chatbotPersona = {
    title: "Triết học Mác",
    openingMessage: "Chào bạn. Chúng ta đang kiếm tiền để thực sự 'sống', hay chỉ đang 'tồn tại' để kiếm ăn qua ngày? Đây là một câu hỏi lớn, và triết học Mác có thể cho chúng ta một góc nhìn sâu sắc. Bạn nghĩ sao về vòng lặp 'làm-tiêu-làm' trong cuộc sống hiện đại?",
    backgroundImage: marxBg
};
...
export const ChatWindow: React.FC<ChatWindowProps> = ({ isDarkMode, toggleDarkMode }) => {
  const [messages, setMessages] = useState<ChatMessageType[]>([
    {
      author: MessageAuthor.MODEL,
      text: chatbotPersona.openingMessage,
      timestamp: new Date()
    }
  ]);
  ...
  }, [userInput, isLoading, toast, messages]);
  ...
  return (
    <div
      ...
      style={{ backgroundImage: `url(${chatbotPersona.backgroundImage})` }}
    >
        ...
        <div className="bg-card/90 backdrop-blur-md border-b border-border/50 p-4 shadow-elegant">
          <div className="flex items-center gap-3">
            ...
            <div className='flex-1'>
              <h2 className="text-lg font-semibold text-foreground">{chatbotPersona.title}</h2>
          ...
          <div className="max-w-4xl mx-auto">
            {messages.map((message, index) => (
              <ChatMessage
                key={index}
                message={message}
                philosopherName={chatbotPersona.title}
              />
            ))}
            {isLoading && (
                <TypingIndicator philosopherName={chatbotPersona.title} />
            )}
